...
async function getMichelinDatabase(): Promise<SQLite.SQLiteDatabase> {
  ...
  michelinDatabaseInitPromise = (async () => {
    ...
    try {
      ...
      const destFile = new File(Paths.document, `michelin_reference_${versionComponent}.db`);
      ...
      for (const entry of Paths.document.list()) {
        if (
          ...
          entry.name !== destFile.name &&
          (entry.name === "michelin_reference.db" ||
            (entry.name.startsWith("michelin_reference_") && entry.name.endsWith(".db")))
        ) {
          try {
            entry.delete();
          } catch (error) {
      ...
      // on every process launch so edits to the bundled database are visible.
      if (destFile.exists && !asset.hash) {
        destFile.delete();
      }
