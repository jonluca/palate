...
import type { MichelinImportSourceDescriptor } from "@/utils/db/michelin-import-core";
...
let michelinReferenceFilePromise: Promise<File> | null = null;
const unversionedDevelopmentDatasetVersion = `development-${Date.now()}`;

/** Content identity for the bundled reference database, generated by Metro. */
export function getMichelinDatasetVersion(): string {
  const asset = Asset.fromModule(michelinDb);
  // Metro development assets may not expose a content hash. Use one stable token
  // per JS process so a restart refreshes both the copied DB and the main DB import.
  return asset.hash ?? unversionedDevelopmentDatasetVersion;
}
...
/**
 * Materialize the bundled guide at one content-addressed, app-controlled path.
 * Both the legacy reader and the native ATTACH importer consume this exact file.
 */
async function getMichelinReferenceFile(): Promise<File> {
  if (michelinReferenceFilePromise) {
    return michelinReferenceFilePromise;
  }
  ...
  const preparation = (async () => {
    ...
    if (!expectedMd5 || !/^[0-9a-f]{32}$/.test(expectedMd5)) {
      throw new Error("Could not verify the Michelin database asset content hash");
    }

    // Include Metro's content hash in the filename so an app update cannot keep
    // opening an obsolete copy left in the document directory.
    const versionComponent = (asset.hash ?? "development").replace(/[^a-zA-Z0-9_-]/g, "_");
    const destination = new File(Paths.document, `michelin_reference_${versionComponent}.db`);
    const staging = new File(Paths.document, `${destination.name}.partial`);

    // Each guide is ~57 MB. Keep only the active reference copy; historical
    // restaurants remain in the main app DB, so old reference files add no value.
    for (const entry of Paths.document.list()) {
      if (
        entry instanceof File &&
        entry.name !== destination.name &&
        (entry.name === "michelin_reference.db" ||
          (entry.name.startsWith("michelin_reference_") &&
            (entry.name.endsWith(".db") || entry.name.endsWith(".db.partial"))))
      ) {
        try {
          entry.delete();
        } catch (error) {
          console.warn(`[MichelinDB] Could not remove stale reference copy ${entry.name}:`, error);
        }
      }
    }

    // Development assets may not have a content hash. Refresh that fallback copy
    // once per process so edits to the bundled database are visible.
    if (destination.exists && !asset.hash) {
      destination.delete();
    }
    if (destination.exists && destination.md5?.toLowerCase() !== expectedMd5) {
      destination.delete();
    }
    if (staging.exists) {
      staging.delete();
    ...
    if (!destination.exists) {
      ...
      try {
        ...
        if (staging.md5?.toLowerCase() !== expectedMd5) {
          throw new Error("Copied Michelin database did not match the bundled asset");
        }
  ...
  michelinReferenceFilePromise = preparation;

  try {
    return await preparation;
  } catch (error) {
    if (michelinReferenceFilePromise === preparation) {
      michelinReferenceFilePromise = null;
    }
    throw error;
  }
}
...
/**
 * Prepare the source descriptor used by the set-based importer. URI parameters
 * make SQLite itself enforce read-only access; `immutable=1` is safe because the
 * copied, content-addressed guide has no WAL writer.
 */
export async function prepareMichelinImportSource(): Promise<MichelinImportSourceDescriptor> {
  const source = await getMichelinReferenceFile();
  if (!source.uri.startsWith("file:") || /[?#]/.test(source.uri)) {
    throw new Error("Michelin reference asset did not resolve to a safe local file URI");
  }
  return {
    datasetVersion: getMichelinDatasetVersion(),
    immutableReadOnlyUri: `${source.uri}?mode=ro&immutable=1&cache=private`,
  };
}
...
async function getMichelinDatabase(): Promise<SQLite.SQLiteDatabase> {
  ...
  michelinDatabaseInitPromise = (async () => {
    ...
    try {
      const referenceFile = await getMichelinReferenceFile();
...
async function readAwardsForDateOrThrow(
  michelinIds: readonly string[],
  timestamp: number,
...
/**
 * Provider-import-only batch reader. Database initialization and query errors
 * reject so the importer can isolate failures; an all-null record is a valid,
 * successful result when no requested restaurant has award history.
 */
export async function readAwardsForProviderImportOrThrow(
  michelinIds: string[],
  timestamp: number,
): Promise<Record<string, string | null>> {
  return readAwardsForDateOrThrow(michelinIds, timestamp);
}
...
export async function getAwardForDate(
  ...
  try {
    ...
    const result = await readAwardsForDateOrThrow(michelinIds, timestamp);
