...
struct PhotoAssetThumbnailCoreTests {
  ...
  func renderDescriptorOptions() throws {
    ...
    #expect(requestOptions.isSynchronous == false)
    #expect(requestOptions.version == .current)
    #expect(requestOptions.deliveryMode == .highQualityFormat)
    #expect(requestOptions.resizeMode == .exact)
    #expect(requestOptions.normalizedCropRect == .zero)
