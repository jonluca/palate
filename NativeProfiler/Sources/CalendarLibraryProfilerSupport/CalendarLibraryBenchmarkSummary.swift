...
public struct CalendarLibraryBenchmarkSummary: Encodable, Equatable, Sendable {
  public let minimumMilliseconds: Double
  public let medianMilliseconds: Double
  public let p95Milliseconds: Double
  public let maximumMilliseconds: Double
  public let medianUniqueEventsPerSecond: Double

  public static func calculate(
    milliseconds: [Double],
    uniqueEventCount: Int
  ) -> CalendarLibraryBenchmarkSummary {
    ...
    let eventsPerSecond = median > 0 ? Double(uniqueEventCount) / (median / 1_000) : 0
    return CalendarLibraryBenchmarkSummary(
      minimumMilliseconds: sorted[0],
      medianMilliseconds: median,
      p95Milliseconds: sorted[p95Index],
      maximumMilliseconds: sorted[sorted.count - 1],
      medianUniqueEventsPerSecond: eventsPerSecond
    )
  }
}
