...
Reports separate the first and all-strip-renderable times from all-final and per-item final p50/p95/max timing. Both strategies use high-quality-only delivery, so renderable and final should coincide; retained degraded counts make any delivery-mode regression visible. Reports also retain final decoded dimensions and pixel totals, RSS checkpoints/peaks, and the candidate store's complete fetch-scheduler and preheat metrics before and after cleanup. RSS baselines are captured after the per-run loader/store is constructed, matching an already-available production store more closely; they remain same-process diagnostics contaminated by allocator and PhotoKit state from prior runs. The run fails on any degraded delivery, missing/failing/timed-out results, invalid dimensions, unexpected or stale callbacks, digest mismatch, a non-quiescent scheduler, any preheat use, incomplete factorial balance, or raw identifier leakage. Validation booleans are derived from the completed measurements and plan before publication; aggregate JSON contains no Photos identifiers, URIs, dates, or filenames and is retained mode 0600.
