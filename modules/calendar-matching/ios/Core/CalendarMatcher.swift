...
public enum CalendarMatcher {
      ...
      if !visit.suggestedRestaurants.isEmpty {
        ...
        matching: for useFuzzyMatching in [false, true] {
          for candidate in candidates {
            ...
            if let cachedTitle = preparedEventTitles[indexedEvent.originalIndex] {
              preparedTitle = cachedTitle
            } else {
              let newTitle = prepareEventTitle(indexedEvent.event.title)
              preparedEventTitles[indexedEvent.originalIndex] = newTitle
  ...
  private static func rankedCandidates(
    ...
    candidates.reserveCapacity(max(0, endIndex - startIndex))
    ...
    candidates.sort { first, second in
      ...
      return first.indexedEvent.originalIndex < second.indexedEvent.originalIndex
    }
