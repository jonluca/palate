...
/**
 * Search for places using a text query within a region.
 * Results are cached indefinitely based on query, location, and radius.
 *
 * @param query - Natural language search query (e.g., "pizza", "sushi near me")
 * @param latitude - Latitude of the search center
 * @param longitude - Longitude of the search center
...
async function searchByText(
  ...
  const key = textSearchKey(query, latitude, longitude, radiusMeters);
  ...
  // Create and cache the promise
  const promise = MapKitSearchModule.searchByText(query, latitude, longitude, radiusMeters) as Promise<
    MapKitSearchResult[]
  >;
