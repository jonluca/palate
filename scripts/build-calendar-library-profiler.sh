...
"$SWIFT_BIN" build \
  ...
  --product PalateCalendarLibraryProfiler

BIN_PATH="$("$SWIFT_BIN" build --package-path "$ROOT_DIR" --configuration release --show-bin-path)"
PROFILER_BINARY="$BIN_PATH/PalateCalendarLibraryProfiler"
APP_PATH="${PALATE_CALENDAR_PROFILER_APP:-$ROOT_DIR/.build/PalateCalendarLibraryProfiler.app}"
BUNDLE_IDENTIFIER="com.jonluca.palate.calendar-library-profiler"
CODE_SIGN_IDENTITY="${PALATE_CALENDAR_PROFILER_CODE_SIGN_IDENTITY:--}"
...
/usr/bin/install -m 755 \
  "$PROFILER_BINARY" \
  "$APP_PATH/Contents/MacOS/PalateCalendarLibraryProfiler"

INFO_PLIST="$APP_PATH/Contents/Info.plist"
/usr/bin/plutil -create xml1 "$INFO_PLIST"
/usr/bin/plutil -insert CFBundleDisplayName -string "Palate Calendar Library Profiler" "$INFO_PLIST"
/usr/bin/plutil -insert CFBundleExecutable -string "PalateCalendarLibraryProfiler" "$INFO_PLIST"
...
/usr/bin/plutil -insert CFBundleName -string "PalateCalendarLibraryProfiler" "$INFO_PLIST"
...
/usr/bin/plutil -insert NSCalendarsUsageDescription -string \
  "Palate Calendar Library Profiler reads event timing metadata to measure native Calendar performance." \
  "$INFO_PLIST"
