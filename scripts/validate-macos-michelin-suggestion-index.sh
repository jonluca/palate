...
ORACLE_HELPER_PATH="$SCRIPT_DIRECTORY/macos-michelin-suggestion-index-oracle.ts"
...
usage() {
  print "Usage: validate-macos-michelin-suggestion-index.sh --app=PATH --database=PATH --output-prefix=PATH [options]"
  print ""
  print "  --timeout-seconds=N    Completion timeout after the timestamped trigger (default: 180)"
  ...
  print ""
  print "The validator stops Palate, publishes a durable byte-exact guard for the live"
  print "main/WAL/SHM/journal set and launch environment, and installs only a disposable"
  print "copy. It removes michelin_suggestion_version and clears pending suggestions so"
  print "the production Rescan Photos prefix must rebuild them. After READY, create the"
  print "printed .trigger file with a fresh fractional epoch immediately before tapping"
  print "Rescan Now. The report is aggregate-only; raw private copies are always deleted."
  print "For --manual-launch, quit and reopen Xcode after READY_TO_LAUNCH so it inherits"
  print "the run ID, then Run Without Building the exact Release product."
}
...
assert_database_unopened_path() {
  local database_path="$1" holder_output
  local -a paths=()
  local suffix
...
durability_sync() {
  ...
  if [[ -n "${PALATE_MICHELIN_SUGGESTION_HARNESS_FAKE_STATE:-}" \
    && -d "$PALATE_MICHELIN_SUGGESTION_HARNESS_FAKE_STATE" \
    && "${PALATE_MICHELIN_SUGGESTION_HARNESS_TEST_FAIL_DURABILITY_PHASE:-}" == "$phase" ]]; then
    print -u2 "Injected durability failure: $phase"
    return 1
  fi
  if [[ "${PALATE_MICHELIN_SUGGESTION_HARNESS_TEST_SKIP_DURABILITY_SYNC:-0}" == "1" \
    && -n "${PALATE_MICHELIN_SUGGESTION_HARNESS_FAKE_STATE:-}" \
    && -d "$PALATE_MICHELIN_SUGGESTION_HARNESS_FAKE_STATE" ]]; then
    return 0
  fi
  /bin/sync
}
...
recover_stale_guard() {
  ...
  local environment_key="PALATE_MICHELIN_SUGGESTION_VALIDATION_RUN_ID"
  ...
  local environment_was_set environment_value
  local created_by_run_id install_temp normal_restore_main normal_restore_wal normal_restore_shm normal_restore_journal
  local main_temp="$guard_path/recovery-main.tmp"
  local wal_temp="$guard_path/recovery-wal.tmp"
  local shm_temp="$guard_path/recovery-shm.tmp"
  local journal_temp="$guard_path/recovery-journal.tmp"
  local failed=0 guard_removed=0

  for dependency in awk head jq launchctl lockf lsof pgrep pkill ps shasum stat tr; do
  ...
  if ! jq -e --arg databasePath "$database_path" --arg environmentKey "$environment_key" '
    ...
    and .kind == "palate-michelin-suggestion-index"
    ...
    and (.launchEnvironment | keys) == [$environmentKey]
    and (.launchEnvironment[$environmentKey].wasSet | type) == "boolean"
    and (.launchEnvironment[$environmentKey].value | type) == "string"
  ...
  environment_was_set="$(jq -r --arg key "$environment_key" 'if .launchEnvironment[$key].wasSet then 1 else 0 end' "$manifest_path")"
  environment_value="$(jq -r --arg key "$environment_key" '.launchEnvironment[$key].value' "$manifest_path")"
  ...
  install_temp="$database_path.install-$created_by_run_id.tmp"
  normal_restore_main="$database_path.restore-$created_by_run_id.main.tmp"
  normal_restore_wal="$database_path.restore-$created_by_run_id.wal.tmp"
  normal_restore_shm="$database_path.restore-$created_by_run_id.shm.tmp"
  normal_restore_journal="$database_path.restore-$created_by_run_id.journal.tmp"

  stop_palate || failed=1
  (( ! failed )) && assert_database_unopened_path "$database_path" || failed=1
  (( ! failed )) && verify_component "$guard_path/main" 1 "$main_hash" 600 "Protected main" || failed=1
  (( ! failed )) && verify_component "$guard_path/wal" "$wal_present" "$wal_hash" 600 "Protected WAL" || failed=1
  (( ! failed )) && verify_component "$guard_path/shm" "$shm_present" "$shm_hash" 600 "Protected SHM" || failed=1
  (( ! failed )) && verify_component "$guard_path/journal" "$journal_present" "$journal_hash" 600 "Protected journal" || failed=1
  (( ! failed )) && prepare_restore_component "$guard_path/main" "$main_temp" "$main_hash" || failed=1
  if (( ! failed && wal_present )) && ! prepare_restore_component "$guard_path/wal" "$wal_temp" "$wal_hash"; then
    failed=1
  fi
  if (( wal_present )) && [[ ! -f "$wal_temp" ]]; then failed=1; fi
  if (( ! failed && shm_present )) && ! prepare_restore_component "$guard_path/shm" "$shm_temp" "$shm_hash"; then
    failed=1
  fi
  if (( shm_present )) && [[ ! -f "$shm_temp" ]]; then failed=1; fi
  if (( ! failed && journal_present )) \
    && ! prepare_restore_component "$guard_path/journal" "$journal_temp" "$journal_hash"; then
    failed=1
  fi
  if (( journal_present )) && [[ ! -f "$journal_temp" ]]; then failed=1; fi

  if (( ! failed )); then
    rm -f -- "$database_path" "$database_path-wal" "$database_path-shm" "$database_path-journal" || failed=1
  fi
  if (( ! failed )); then
    mv -f -- "$main_temp" "$database_path" && chmod "$main_mode" "$database_path" || failed=1
    if (( wal_present )); then mv -f -- "$wal_temp" "$database_path-wal" && chmod "$wal_mode" "$database_path-wal" || failed=1; fi
    if (( shm_present )); then mv -f -- "$shm_temp" "$database_path-shm" && chmod "$shm_mode" "$database_path-shm" || failed=1; fi
    if (( journal_present )); then mv -f -- "$journal_temp" "$database_path-journal" && chmod "$journal_mode" "$database_path-journal" || failed=1; fi
  fi
  (( ! failed )) && verify_component "$database_path" 1 "$main_hash" "$main_mode" "Restored main" || failed=1
  (( ! failed )) && verify_component "$database_path-wal" "$wal_present" "$wal_hash" "$wal_mode" "Restored WAL" || failed=1
  (( ! failed )) && verify_component "$database_path-shm" "$shm_present" "$shm_hash" "$shm_mode" "Restored SHM" || failed=1
  (( ! failed )) && verify_component "$database_path-journal" "$journal_present" "$journal_hash" "$journal_mode" "Restored journal" || failed=1
  (( ! failed )) && durability_sync recovery-restored-database || failed=1

  if (( ! failed )); then
    if (( environment_was_set )); then
      launchctl setenv "$environment_key" "$environment_value" \
        && [[ "$(launchctl getenv "$environment_key" 2>/dev/null || true)" == "$environment_value" ]] \
        && launch_environment_key_is_set "$environment_key" || failed=1
    else
      launchctl unsetenv "$environment_key" || failed=1
      launch_environment_key_is_set "$environment_key" && failed=1
    fi
  fi
  if (( ! failed )); then
    rm -f -- "$install_temp" "$normal_restore_main" "$normal_restore_wal" \
      "$normal_restore_shm" "$normal_restore_journal" || failed=1
    if [[ -e "$install_temp" || -e "$normal_restore_main" || -e "$normal_restore_wal" \
      || -e "$normal_restore_shm" || -e "$normal_restore_journal" ]]; then
      failed=1
    fi
  fi
  if (( ! failed )); then
  ...
  rm -f -- "$main_temp" "$wal_temp" "$shm_temp" "$journal_temp" \
    "$install_temp" "$normal_restore_main" "$normal_restore_wal" \
    "$normal_restore_shm" "$normal_restore_journal" || true
  if (( failed )); then
...
if (( ! MANUAL_LAUNCH )) \
  && [[ "${PALATE_MICHELIN_SUGGESTION_HARNESS_ALLOW_DIRECT_OPEN:-0}" != "1" ]]; then
  print -u2 "Real signed validation requires --manual-launch; direct open is test-only"
  exit 2
fi

NODE_BINARY="${PALATE_NODE_BINARY:-node}"
for dependency in awk codesign jq launchctl lockf lsof md5 open pgrep pkill ps rg shasum sqlite3 stat tr "$NODE_BINARY"; do
  command -v "$dependency" >/dev/null 2>&1 || {
...
RUN_ID="michelin-suggestion-$$-$(date +%s)-$RANDOM"
ENVIRONMENT_KEY="PALATE_MICHELIN_SUGGESTION_VALIDATION_RUN_ID"
...
ORACLE_PATH="$GUARD_PATH/oracle.json"
COMPARISON_PATH="$GUARD_PATH/comparison.json"
...
ORIGINAL_ENVIRONMENT_VALUE="$(launchctl getenv "$ENVIRONMENT_KEY" 2>/dev/null || true)"
ORIGINAL_ENVIRONMENT_WAS_SET=0
launch_environment_key_is_set "$ENVIRONMENT_KEY" && ORIGINAL_ENVIRONMENT_WAS_SET=1
...
capture_guard() {
  local suffix source destination hash mode size present
  stop_palate
  ...
  for suffix in wal shm journal; do
    ...
    if [[ -e "$source" || -L "$source" ]]; then
      ...
      case "$suffix" in
        wal)
          ORIGINAL_WAL_PRESENT=1; ORIGINAL_WAL_SHA256="$hash"; ORIGINAL_WAL_MODE="$mode"; ORIGINAL_WAL_SIZE="$size"
          ;;
        shm)
          ORIGINAL_SHM_PRESENT=1; ORIGINAL_SHM_SHA256="$hash"; ORIGINAL_SHM_MODE="$mode"; ORIGINAL_SHM_SIZE="$size"
          ;;
        journal)
          ORIGINAL_JOURNAL_PRESENT=1; ORIGINAL_JOURNAL_SHA256="$hash"; ORIGINAL_JOURNAL_MODE="$mode"; ORIGINAL_JOURNAL_SIZE="$size"
          ;;
      esac
    fi
  done
  jq -n \
    ...
    --arg runId "$RUN_ID" \
    --arg environmentKey "$ENVIRONMENT_KEY" \
    --arg environmentValue "$ORIGINAL_ENVIRONMENT_VALUE" \
    --argjson environmentWasSet "$ORIGINAL_ENVIRONMENT_WAS_SET" \
    ...
    '{
      schemaVersion: 1,
      kind: "palate-michelin-suggestion-index",
      ...
      launchEnvironment: {($environmentKey): {wasSet: ($environmentWasSet == 1), value: $environmentValue}}
...
restore_environment() {
  ...
  if (( ORIGINAL_ENVIRONMENT_WAS_SET )); then
    launchctl setenv "$ENVIRONMENT_KEY" "$ORIGINAL_ENVIRONMENT_VALUE"
    [[ "$(launchctl getenv "$ENVIRONMENT_KEY" 2>/dev/null || true)" == "$ORIGINAL_ENVIRONMENT_VALUE" ]]
    launch_environment_key_is_set "$ENVIRONMENT_KEY"
  else
    launchctl unsetenv "$ENVIRONMENT_KEY"
    ! launch_environment_key_is_set "$ENVIRONMENT_KEY"
  fi
...
restore_database() {
  ...
  stop_palate || restore_failed=1
  (( ! restore_failed )) && assert_database_unopened_path "$DATABASE_PATH" || restore_failed=1
  (( ! restore_failed )) \
    && verify_component "$GUARD_MAIN_PATH" 1 "$ORIGINAL_MAIN_SHA256" 600 "Protected main" \
    || restore_failed=1
  (( ! restore_failed )) \
    && verify_component "$GUARD_WAL_PATH" "$ORIGINAL_WAL_PRESENT" "$ORIGINAL_WAL_SHA256" 600 "Protected WAL" \
    || restore_failed=1
  (( ! restore_failed )) \
    && verify_component "$GUARD_SHM_PATH" "$ORIGINAL_SHM_PRESENT" "$ORIGINAL_SHM_SHA256" 600 "Protected SHM" \
    || restore_failed=1
  (( ! restore_failed )) \
    && verify_component "$GUARD_JOURNAL_PATH" "$ORIGINAL_JOURNAL_PRESENT" "$ORIGINAL_JOURNAL_SHA256" 600 "Protected journal" \
    || restore_failed=1
  (( ! restore_failed )) \
    && prepare_restore_component "$GUARD_MAIN_PATH" "$RESTORE_MAIN_TEMP_PATH" "$ORIGINAL_MAIN_SHA256" \
    || restore_failed=1
  if (( ! restore_failed && ORIGINAL_WAL_PRESENT )); then
    prepare_restore_component "$GUARD_WAL_PATH" "$RESTORE_WAL_TEMP_PATH" "$ORIGINAL_WAL_SHA256" \
      || restore_failed=1
  fi
  if (( ! restore_failed && ORIGINAL_SHM_PRESENT )); then
    prepare_restore_component "$GUARD_SHM_PATH" "$RESTORE_SHM_TEMP_PATH" "$ORIGINAL_SHM_SHA256" \
      || restore_failed=1
  fi
  if (( ! restore_failed && ORIGINAL_JOURNAL_PRESENT )); then
    prepare_restore_component "$GUARD_JOURNAL_PATH" "$RESTORE_JOURNAL_TEMP_PATH" "$ORIGINAL_JOURNAL_SHA256" \
      || restore_failed=1
  fi
  ...
  rm -f -- "$DATABASE_PATH" "$DATABASE_PATH-wal" "$DATABASE_PATH-shm" "$DATABASE_PATH-journal" \
    || restore_failed=1
  if (( ! restore_failed )); then
    mv -f -- "$RESTORE_MAIN_TEMP_PATH" "$DATABASE_PATH" \
      && chmod "$ORIGINAL_MAIN_MODE" "$DATABASE_PATH" \
      || restore_failed=1
  fi
  if (( ! restore_failed && ORIGINAL_WAL_PRESENT )); then
    mv -f -- "$RESTORE_WAL_TEMP_PATH" "$DATABASE_PATH-wal" \
      && chmod "$ORIGINAL_WAL_MODE" "$DATABASE_PATH-wal" \
      || restore_failed=1
  fi
  if (( ! restore_failed && ORIGINAL_SHM_PRESENT )); then
    mv -f -- "$RESTORE_SHM_TEMP_PATH" "$DATABASE_PATH-shm" \
      && chmod "$ORIGINAL_SHM_MODE" "$DATABASE_PATH-shm" \
      || restore_failed=1
  fi
  if (( ! restore_failed && ORIGINAL_JOURNAL_PRESENT )); then
    mv -f -- "$RESTORE_JOURNAL_TEMP_PATH" "$DATABASE_PATH-journal" \
      && chmod "$ORIGINAL_JOURNAL_MODE" "$DATABASE_PATH-journal" \
      || restore_failed=1
  fi
  (( ! restore_failed )) \
    && verify_component "$DATABASE_PATH" 1 "$ORIGINAL_MAIN_SHA256" "$ORIGINAL_MAIN_MODE" "Restored main" \
    || restore_failed=1
  (( ! restore_failed )) \
    && verify_component "$DATABASE_PATH-wal" "$ORIGINAL_WAL_PRESENT" "$ORIGINAL_WAL_SHA256" "$ORIGINAL_WAL_MODE" "Restored WAL" \
    || restore_failed=1
  (( ! restore_failed )) \
    && verify_component "$DATABASE_PATH-shm" "$ORIGINAL_SHM_PRESENT" "$ORIGINAL_SHM_SHA256" "$ORIGINAL_SHM_MODE" "Restored SHM" \
    || restore_failed=1
  (( ! restore_failed )) \
    && verify_component "$DATABASE_PATH-journal" "$ORIGINAL_JOURNAL_PRESENT" "$ORIGINAL_JOURNAL_SHA256" "$ORIGINAL_JOURNAL_MODE" "Restored journal" \
    || restore_failed=1
  (( ! restore_failed )) && durability_sync restored-database || restore_failed=1
...
cleanup() {
  ...
  if (( ! SUCCESS )); then
    rm -f -- "$REPORT_TEMP_PATH" "$REPORT_PATH" || true
  fi
  CLEANUP_RUNNING=0
  if (( failed )); then
...
"$NODE_BINARY" --no-warnings --experimental-sqlite --experimental-strip-types \
  "$ORACLE_HELPER_PATH" prepare \
  --database="$PREPARED_DATABASE_PATH" \
  --guide="$APP_GUIDE_PATH" \
  --dataset-version="$APP_GUIDE_DATASET_VERSION" \
  --oracle="$ORACLE_PATH"
chmod 600 "$ORACLE_PATH"
EXPECTED_SUGGESTION_VERSION="$(jq -r '.suggestionVersion' "$ORACLE_PATH")"
EXPECTED_PENDING_VISITS="$(jq '.pendingVisits | length' "$ORACLE_PATH")"
EXPECTED_PENDING_SUGGESTIONS="$(jq '.expectedPendingSuggestions | length' "$ORACLE_PATH")"
EXPECTED_PENDING_PRIMARY="$(jq '[.expectedPendingPrimary[] | select(.restaurantId != null)] | length' "$ORACLE_PATH")"
...
UPDATE visits SET suggestedRestaurantId = NULL WHERE status = 'pending';
DELETE FROM app_metadata WHERE key = 'michelin_suggestion_version';
COMMIT;
PRAGMA wal_checkpoint(TRUNCATE);
PRAGMA journal_mode = DELETE;
SQL
rm -f -- "$PREPARED_DATABASE_PATH-wal" "$PREPARED_DATABASE_PATH-shm" "$PREPARED_DATABASE_PATH-journal"
[[ "$(sqlite3 "$PREPARED_DATABASE_PATH" "SELECT COUNT(*) FROM app_metadata WHERE key='michelin_suggestion_version';")" == "0" ]]
[[ "$(sqlite3 "$PREPARED_DATABASE_PATH" "SELECT COUNT(*) FROM visit_suggested_restaurants vsr JOIN visits v ON v.id=vsr.visitId WHERE v.status='pending';")" == "0" ]]
[[ "$(sqlite3 "$PREPARED_DATABASE_PATH" "SELECT COUNT(*) FROM visits WHERE status='pending' AND suggestedRestaurantId IS NOT NULL;")" == "0" ]]

launchctl setenv "$ENVIRONMENT_KEY" "$RUN_ID"
[[ "$(launchctl getenv "$ENVIRONMENT_KEY" 2>/dev/null || true)" == "$RUN_ID" ]]
launch_environment_key_is_set "$ENVIRONMENT_KEY"
...
print "READY_TO_LAUNCH run_id=$RUN_ID app=$APP_PATH"
if (( ! MANUAL_LAUNCH )); then
  PALATE_MICHELIN_SUGGESTION_HARNESS_FAKE_DATABASE="$DATABASE_PATH" \
    PALATE_MICHELIN_SUGGESTION_HARNESS_FAKE_TRIGGER="$TRIGGER_PATH" \
    PALATE_MICHELIN_SUGGESTION_HARNESS_FAKE_ORACLE="$ORACLE_PATH" \
    PALATE_MICHELIN_SUGGESTION_HARNESS_FAKE_NODE="$NODE_BINARY" \
    PALATE_MICHELIN_SUGGESTION_HARNESS_FAKE_ORACLE_HELPER="$ORACLE_HELPER_PATH" \
    open -n "$APP_PATH"
fi
...
PROCESS_COMMAND="$(ps eww -p "$APP_PID" -o command= 2>/dev/null || true)"
if [[ " $PROCESS_COMMAND " != *" $ENVIRONMENT_KEY=$RUN_ID "* ]]; then
  print -u2 "Running Palate did not inherit the validation run ID"
  exit 1
fi
...
print "READY run_id=$RUN_ID trigger=$TRIGGER_PATH action='Settings > Advanced Settings > Rescan Photos > Rescan Now'"
...
while (( $(date +%s) <= COMPLETION_DEADLINE )); do
  kill -0 "$APP_PID" 2>/dev/null || {
    print -u2 "Palate exited before Michelin suggestion refresh completed"
    exit 1
  }
  ...
  OBSERVED_VERSION="$(sqlite3 -readonly "$DATABASE_PATH" "SELECT value FROM app_metadata WHERE key='michelin_suggestion_version';" 2>/dev/null || true)"
  if [[ "$OBSERVED_VERSION" == "$EXPECTED_SUGGESTION_VERSION" ]]; then
...
if [[ -z "$COMPLETION_EPOCH" ]]; then
  print -u2 "Timed out waiting for the production Michelin suggestion version stamp"
  exit 1
fi
...
if ! "$NODE_BINARY" --no-warnings --experimental-sqlite --experimental-strip-types \
  ...
  print -u2 "Signed Michelin suggestion refresh did not match the independent oracle"
  exit 1
fi
chmod 600 "$COMPARISON_PATH"
...
jq -n \
  ...
  --arg appBundleName "${APP_PATH:t}" \
  --arg executableSha256 "$APP_EXECUTABLE_SHA256" \
  --arg bundleSha256 "$APP_BUNDLE_SHA256" \
  --arg guideSha256 "$APP_GUIDE_SHA256" \
  --arg guideVersion "$APP_GUIDE_DATASET_VERSION" \
  --arg schemeRunConfiguration "$SCHEME_RUN_CONFIGURATION" \
  --arg sourceMainSha256 "$ORIGINAL_MAIN_SHA256" \
  --arg sourceMainMode "$ORIGINAL_MAIN_MODE" \
  --argjson sourceMainSize "$ORIGINAL_MAIN_SIZE" \
  ...
  --arg triggerEpoch "$TRIGGER_EPOCH" \
  --arg completionEpoch "$COMPLETION_EPOCH" \
  --argjson durationSeconds "$TRIGGER_TO_COMPLETION_SECONDS" \
  --argjson initialRssKib "$INITIAL_RSS_KIB" \
  --argjson maxRssKib "$MAX_RSS_KIB" \
  --argjson expectedPendingVisits "$EXPECTED_PENDING_VISITS" \
  --argjson expectedPendingSuggestions "$EXPECTED_PENDING_SUGGESTIONS" \
  --argjson expectedPendingPrimary "$EXPECTED_PENDING_PRIMARY" \
  ...
  '{
    ...
    signedBuild: {
      ...
      processEnvironmentRunIdMatched: true,
    ...
    fixture: {
      ...
      suggestionVersionRemoved: true,
      pendingSuggestionsCleared: true,
      pendingVisits: $expectedPendingVisits,
      expectedSuggestionRows: $expectedPendingSuggestions,
      expectedPrimarySuggestions: $expectedPendingPrimary
    },
    timing: {
      timestampedManualTrigger: true,
      triggerEpochSeconds: ($triggerEpoch | tonumber),
      completionObservedEpochSeconds: ($completionEpoch | tonumber),
      triggerToSuggestionVersionSeconds: $durationSeconds,
    ...
    restoration: {
      ...
      exactLaunchEnvironmentState: true,
...
rm -f -- "$TRIGGER_PATH"
SUCCESS=1
print "VALIDATED report=$REPORT_PATH duration_seconds=$TRIGGER_TO_COMPLETION_SECONDS pending_visits=$EXPECTED_PENDING_VISITS suggestions=$EXPECTED_PENDING_SUGGESTIONS"
