...
Deep scans process only pending photos. Per-photo errors and missing native results retry automatically within the same scan, with short delays and at most three total failed attempts per photo. Each failure is persisted; retries spend only the remaining budget and never include photos that already succeeded. Exhausted photos retain their unknown classification but leave manual, automatic, and visit-sampling pending work. Completion waits for the Settings pending count to refresh. An explicit scan of a visit's photos can retry an exhausted photo; a successful classification clears its failure count. Whole-batch errors do not consume individual photos' retry budgets.
...
The baseline is specifically the underlying Expo `PhotoLibraryAssetLoader` PhotoKit request behavior—per-item `PHAsset` fetch plus `PHImageManager.default()` high-quality/fast/aspect-fit at a cover target. The candidate is the shared `PhotoAssetThumbnailStore` high-quality-only/exact/aspect-fill path. Reports compare all-strip-renderable and all-final latency, decoded pixels, RSS, and full store/scheduler metrics; strict validation rejects degraded callbacks, failures, timeouts, stale events, bad dimensions/digests, preheat use, non-quiescence, or raw identifier leakage. This does not claim warm `expo-image`/SDWebImage cache parity. Cancel/resubmit and warm-revisit arms are explicitly deferred.

The previous July report at `.build/preview-cards-profile-commits-20260711T052431Z.json` is retained only as historical evidence for the opportunistic path. The high-quality diagnostic at `.build/preview-cards-high-quality.json` (SHA-256 `aed88a3368f3b0729ea6e282e1bdf91283e13b99116ace7da09ba80154ec0163`) covered **192 globally disjoint assets: 168 images and 24 videos**. It recorded **zero degraded callbacks, failures, or timeouts**. Median cold PhotoKit completion fell from **274.180 ms to 62.121 ms (4.414×)**, while candidate decoded pixels were **23.20%** of baseline. Same-process median sampled peak RSS deltas were 9,920,512 versus 172,032 bytes, but allocator and PhotoKit caches carry across arms, so that memory comparison remains diagnostic only. This harness does not instantiate `expo-image` or FlashList and excludes SDWebImage caches, warm revisits, and recycled-view display latency, so it is not promotion evidence for the restaurant feed. The feed retains its long-lived direct `expo-image` renderer; the native thumbnail store remains available to other surfaces and isolated profiling.
