...
The previous July report at `.build/preview-cards-profile-commits-20260711T052431Z.json` is retained only as historical evidence for the opportunistic path. The post-switch high-quality diagnostic at `.build/preview-cards-high-quality.json` (SHA-256 `aed88a3368f3b0729ea6e282e1bdf91283e13b99116ace7da09ba80154ec0163`) covered **192 globally disjoint assets: 168 images and 24 videos**. It recorded **zero degraded callbacks, failures, or timeouts**. Median full-strip final/renderable completion fell from **274.180 ms to 62.121 ms (4.414×)**, while candidate decoded pixels were **23.20%** of baseline. Same-process median sampled peak RSS deltas were 9,920,512 versus 172,032 bytes, but allocator and PhotoKit caches carry across arms, so that memory comparison remains diagnostic only. The production feed now uses this exact-size native renderer without speculative preheat; a native rebuild is required because the delivery mode lives in Swift.
